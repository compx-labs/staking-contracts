...
describe('Injected Reward Pool injection test - no staking', () => {
  ...
  test('freeze rewards', async () => {
    ...
    console.log('freezreValue', freezreValue);
  ...
  test('inject rewards ASA 1', async () => {
    ...
    for (var staker of stakingAccounts) {
      ...
      const accruedRewardsValuesBefore: bigint[] = getByteArrayValuesAsBigInts(accruedRewards, BYTE_LENGTH_REWARD_ASSET);
      console.log('accruedRewards', staker.account?.addr, accruedRewardsValuesBefore);
      expect(accruedRewardsValuesBefore[0]).toBe(0n);
    }
    ...
    for (var staker of stakingAccounts) {
      const localState = await appClient.getLocalState(staker.account!.addr);
      const accruedRewards = localState.accruedRewards!.asByteArray();
      const accruedRewardsValuesAfter: bigint[] = getByteArrayValuesAsBigInts(accruedRewards, BYTE_LENGTH_REWARD_ASSET);
      console.log('accruedRewards', staker.account?.addr, accruedRewardsValuesAfter);
      expect(accruedRewardsValuesAfter[0]).toBe(0n);
    }

    await appClient.setFreeze({ enabled: false });
    const globalStateAfterUnFreeze = await appClient.getGlobalState();
    const freezeState = globalStateAfterUnFreeze.freeze!.asByteArray();
    const freezreValue = byteArrayToUint128(freezeState);
    console.log('freezreValue', freezreValue);
    ...
    for (var staker of stakingAccounts) {
      ...
      const accruedRewardsValuesAfter: bigint[] = getByteArrayValuesAsBigInts(accruedRewards, BYTE_LENGTH_REWARD_ASSET);
      console.log('accruedRewards', staker.account?.addr, accruedRewardsValuesAfter);
      expect(accruedRewardsValuesAfter[0]).toBeGreaterThan(0n);
    }
  });
  ...
  test('claim rewards', async () => {
    ...
    for (var staker of stakingAccounts) {
      ...
      let claimFees = AlgoAmount.MicroAlgos(240_000);
      const response = await appClient.compose()
        .gas({}, { note: '1' })
        .gas({}, { note: '2' })
        .claimRewards({}, { sender: staker.account, sendParams: { fee: claimFees} })
       // .execute({ populateAppCallResources: true })
      .simulate({ allowUnnamedResources: true, allowMoreLogging: true })

      claimFees = AlgoAmount.MicroAlgos(
        2000 +
        1000 *
        Math.floor(((response.simulateResponse.txnGroups[0].appBudgetAdded as number) + 699) / 700),
      )
      consoleLogger.info(`claimRewards fees:${claimFees.toString()}`)
